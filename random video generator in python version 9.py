...
class InfiniteNoiseImage:
    ...
    def generate_rgb_values(self):
        ...
        for frame in range(self.frames):
            ...
            # Генерация всех комбинаций с повторениями
            repeated_colors = list(product(colors, repeat=self.total_pixels // 10))

            # Перемешивание и выбор случайной комбинации
            frame_rgb = random.choice(repeated_colors)

            # Для окончательного массива, чтобы соответствовать размеру изображения
            frame_rgb = list(frame_rgb)[:self.total_pixels]
